...
class FastSearchCluster(object):
    ...
    def calculate_cluster_center(self):
        ...
        """
        gamma = self.gamma
        self.cluster_center = np.where(gamma >= 2.9)[0]

    # def calculate_upper_bound(self, dc):
    #     cluster_temp = self.cluster_temp
    #     for i in range(self.data_size - 1):
    #         for j in range(i + 1, self.data_size):
    #             if cluster_temp[i] != cluster_temp[j] and self.distance[(i, j)] < dc:
    #                 rho_i, rho_j = self.result[i, 0], self.result[j, 0]
    #                 avg_rho, i_bound, j_bound = (rho_i + rho_j) / 2, self.cluster_upper_bound[cluster_temp[i]], self.cluster_upper_bound[cluster_temp[j]]
    #                 self.cluster_upper_bound[cluster_temp[i]] = min(avg_rho, i_bound)
    #                 self.cluster_upper_bound[cluster_temp[j]] = min(avg_rho, j_bound)
    #
    # def get_hole(self):
    #     for index, rho in enumerate(self.result[:, 0]):
    #         if rho < self.cluster_upper_bound[self.cluster_temp[index]]:
    #             self.cluster_hole.append(index)

    def get_center(self, filename, autoDc=False):
...
if __name__ == '__main__':
    ...
    # plot rho and delta
    plot.plot_diagram(pre.result[:, 0], pre.result[:, 1], 'rho', 'delta', 'Decision Graph')
    # plot gamma diagram to get cluster center
    des_gamma = sorted(pre.gamma, reverse=True)
    plot.plot_diagram(np.arange(pre.data_size), des_gamma, 'x', 'gamma', 'gamma diagram')
    # plot the result
    builder = GetDistance()
